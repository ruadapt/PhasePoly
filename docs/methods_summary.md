...
| **Incremental Block Merging Strategy** | Multi-round schedule that starts with single-block optimization and progressively widens `group_size` (e.g. 1 → 3 → 1 → 5 → 1 → 7 → 1), reverting any round that fails to improve. | the default chained-rounds schedule in [`scripts/run_experiment.py`](../scripts/run_experiment.py) (`ROUNDS`) |
...
| Production schedule (Incremental Block Merging) | call [`scripts/run_experiment.py`](../scripts/run_experiment.py) — its in-file `ROUNDS` chains `group_size ∈ {1, 3, 1, 5, 1, 7, 1}` and reverts any round that regresses |
